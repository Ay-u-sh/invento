...
class SQLiteHandler : public DatabaseHandler{
    ...
    public:
        ...
        bool insertData(vector<StructuredData*> data){
            ...
            int result_code_prepare = prepareQuery(query,get_insert_data_stmt_state);
            bool result_process_data;
            
            if(result_code_prepare != SQLITE_OK || !(result_process_data = processInsertData(data)))
                return false;
        ...
        vector<string> getTopSelling(){
            ...
            if(!processResultData(data,get_top_selling_stmt_state));   
...
unsigned char* commonDataParser(vector<string> data,unsigned char* separator){
    ...
    strcpy((char*)sqlite_data,result.c_str());
